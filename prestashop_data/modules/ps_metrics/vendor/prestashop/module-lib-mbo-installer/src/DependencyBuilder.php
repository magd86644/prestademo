...
namespace ps_metrics_module_v4_1_1\Prestashop\ModuleLibMboInstaller;
...
class DependencyBuilder
{
    ...
    {
        $context = \ps_metrics_module_v4_1_1\ContextCore::getContext();
        if ($context !== null && $context->employee !== null) {
            $locale = \ps_metrics_module_v4_1_1\DbCore::getInstance()->getValue(\sprintf('SELECT `locale` FROM `%slang` WHERE `id_lang`=%s', _DB_PREFIX_, pSQL((string) $context->employee->id_lang)));
        }
        if (empty($locale)) {
            return 'en-GB';
        }
    ...
    {
        global $kernel;
        if (!$kernel instanceof \ps_metrics_module_v4_1_1\AppKernel) {
    ...
    private function getDependencies($addRoutes = \false)
    {
        ...
        foreach ($dependenciesContent['dependencies'] as $dependency) {
            ...
            $dependencyData = \ps_metrics_module_v4_1_1\DbCore::getInstance()->getRow(\sprintf('SELECT `id_module`, `active`, `version` FROM `%smodule` WHERE `name` = "%s"', _DB_PREFIX_, pSQL((string) $dependency['name'])));
            if ($dependencyData && \is_array($dependencyData) && !empty($dependencyData['id_module'])) {
                ...
                $isModuleActiveForAllShops = (bool) \ps_metrics_module_v4_1_1\DbCore::getInstance()->getValue(\sprintf('SELECT id_module
                            FROM `%smodule_shop`
