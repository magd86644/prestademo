...
class HttpClient
{
    ...
    {
        $this->options = [\CURLOPT_RETURNTRANSFER => \true, \CURLOPT_FOLLOWLOCATION => \true, \CURLOPT_MAXREDIRS => 5, \CURLOPT_TIMEOUT => 30, \CURLOPT_SSL_VERIFYPEER => \true, \CURLOPT_SSL_VERIFYHOST => 2, \CURLOPT_HTTP_VERSION => \CURL_HTTP_VERSION_1_1];
    }
    /**
    ...
    public function request($method, $url, $data = null)
    {
        ...
        $options = $this->options + [\CURLOPT_URL => $url, \CURLOPT_CUSTOMREQUEST => \strtoupper($method), \CURLOPT_HEADER => \true, \CURLOPT_HTTPHEADER => $this->headers];
        if ($data !== null) {
            if (\is_array($data)) {
        ...
        foreach (\explode("\r\n", $headerStr) as $line) {
            if (\preg_match('/^([^:]+):(.+)$/', $line, $matches)) {
                $headers[\trim($matches[1])] = \trim($matches[2]);
            }
        }
